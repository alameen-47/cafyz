...
export async function ensureFounderAccount(): Promise<void> {
  ...
  if (existing.rows.length) {
    ...
    await db.execute({
      sql: `UPDATE users SET email=?, password_hash=?, restaurant_id=?, status='active' WHERE id=?`,
      args: [email, hash, SYSTEM_REST, id],
    });
  ...
  if (byEmail.rows.length) {
    ...
    const id = String(row.id);
    await db.execute({
      sql: `UPDATE users SET password_hash=?, role='founder', restaurant_id=?, status='active' WHERE id=?`,
      args: [hash, SYSTEM_REST, id],
    });
  ...
  await db.execute({
    sql: `INSERT INTO users(id,restaurant_id,name,initials,email,password_hash,role,status,start_time)
          VALUES(?,?,?,?,?,?,?,?,?)`,
    args: [uid(), SYSTEM_REST, 'Cafyz Founder', 'CF', email, hash, 'founder', 'active', '—'],
  });
