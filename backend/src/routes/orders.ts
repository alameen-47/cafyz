...
router.get('/', async (req: AuthRequest, res, next) => {
  try {
    ...
    let sql = `SELECT o.*, t.name as table_name FROM orders o LEFT JOIN restaurant_tables t ON t.id=o.table_id WHERE 1=1`;
    const args: any[] = [];
...
router.get('/:id', async (req, res, next) => {
  try {
    const db = getDb();
    const order = await db.execute({ sql: 'SELECT * FROM orders WHERE id=?', args: [(req.params.id as string)] });
...
router.post('/', async (req: AuthRequest, res, next) => {
  try {
    ...
    const id = uid();
    await getDb().execute({
      sql: `INSERT INTO orders(id,table_id,server_id,covers,note) VALUES(?,?,?,?,?)`,
      args: [id, data.table_id??null, req.user!.id, data.covers, data.note??null],
    });
    if (data.table_id) {
      await getDb().execute({ sql: `UPDATE restaurant_tables SET status='occupied' WHERE id=?`, args: [data.table_id] });
    }
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const db = getDb();
    const ex = await db.execute({ sql: 'SELECT id FROM orders WHERE id=?', args: [(req.params.id as string)] });
...
router.patch('/:id/status', async (req, res, next) => {
  try {
    ...
    const db = getDb();
    const ex = await db.execute({ sql: 'SELECT * FROM orders WHERE id=?', args: [(req.params.id as string)] });
    if (!ex.rows.length) { res.status(404).json({ error: 'Order not found' }); return; }
    await db.execute({ sql: "UPDATE orders SET status=?,updated_at=datetime('now') WHERE id=?", args: [status, (req.params.id as string)] });
    const order = ex.rows[0] as Record<string,unknown>;
    if ((status === 'paid' || status === 'voided') && order.table_id) {
      await db.execute({ sql: `UPDATE restaurant_tables SET status='empty',course='',covers=0 WHERE id=?`, args: [order.table_id as string] });
    }
...
router.delete('/:id/items/:itemId', async (req, res, next) => {
  try {
    const ex = await getDb().execute({ sql: 'SELECT id FROM order_items WHERE id=? AND order_id=?', args: [(req.params.itemId as string), (req.params.id as string)] });
    if (!ex.rows.length) { res.status(404).json({ error: 'Item not found' }); return; }
...
router.delete('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    const ex = await getDb().execute({ sql: 'SELECT id FROM orders WHERE id=?', args: [(req.params.id as string)] });
    if (!ex.rows.length) { res.status(404).json({ error: 'Order not found' }); return; }
    await getDb().execute({ sql: 'DELETE FROM orders WHERE id=?', args: [(req.params.id as string)] });
    res.status(204).end();
  } catch (e) { next(e); }
});
