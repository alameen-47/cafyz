...
/** Fetch a remote logo (e.g. Cloudinary) and dither it for thermal printing. */
export async function urlToPrintableDataUrl(url: string): Promise<string> {
  const res = await fetch(url, { mode: 'cors' });
  if (!res.ok) throw new Error(`Could not download logo (${res.status}).`);
  const blob = await res.blob();
  if (!blob.type.startsWith('image/')) throw new Error('Logo URL did not return an image.');
  const raw = await new Promise<string>((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => {
      const result = reader.result;
      if (typeof result === 'string' && result.startsWith('data:')) resolve(result);
      else reject(new Error('Could not read logo image.'));
    };
    reader.onerror = () => reject(new Error('Could not read logo image.'));
    reader.readAsDataURL(blob);
  });
  return colorLogoToPrintableDataUrl(raw);
}
...
async function loadImageFromUrl(url: string): Promise<HTMLImageElement> {
  // Prefer fetch→blob so thermal rasterization is not blocked by canvas CORS taint.
  try {
    const printable = await urlToPrintableDataUrl(url);
    return loadImageFromDataUrl(printable);
  } catch {
    return new Promise((resolve, reject) => {
      const img = new Image();
...
/** Stored logo data URL or remote URL → ESC/POS raster. */
export async function logoDataUrlToEscPos(logoUrl: string): Promise<Uint8Array> {
  const img = logoUrl.startsWith('http://') || logoUrl.startsWith('https://')
    ? await loadImageFromUrl(logoUrl)
    : await loadImageFromDataUrl(logoUrl);
  const canvas = drawLogoToCanvas(img, img.naturalWidth, img.naturalHeight);
