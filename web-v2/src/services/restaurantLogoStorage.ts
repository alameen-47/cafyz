...
import { colorLogoToPrintableDataUrl, urlToPrintableDataUrl } from './logoThermalRaster';
...
/** Pull logo from API into local printable cache (call after login / profile load). */
export async function syncRestaurantLogoCacheAsync(restaurant: { id: string; logo_url?: string | null }): Promise<void> {
  if (!restaurant.id) return;
  if (!isValidLogoUrl(restaurant.logo_url)) {
    clearRestaurantLogo(restaurant.id);
    return;
  }
  const url = restaurant.logo_url!;
  if (url.startsWith('data:image/')) {
    try { setRestaurantLogo(restaurant.id, url); } catch { /* storage full */ }
    return;
  }
  try {
    const printable = await urlToPrintableDataUrl(url);
    setRestaurantLogo(restaurant.id, printable);
  } catch {
    // Print path can still fetch the HTTPS URL at runtime.
  }
}

/** @deprecated use syncRestaurantLogoCacheAsync — sync metadata only */
...
export async function refreshRestaurantLogoFromServer(restaurantId: string): Promise<string | undefined> {
  ...
  await syncRestaurantLogoCacheAsync(restaurant);
...
/**
 * Upload restaurant logo: Cloudinary when configured, otherwise dithered data URL in DB.
 * Always caches a printable copy on this device for receipts.
 */
export async function uploadRestaurantLogo(restaurantId: string, file: File): Promise<string> {
  if (!restaurantId) throw new Error('Restaurant not loaded yet — wait a moment and try again.');
  ...
  try {
    ...
    try {
      const printable = await urlToPrintableDataUrl(url);
      setRestaurantLogo(restaurantId, printable);
    } catch {
    ...
    return saveRestaurantLogoFromFile(restaurantId, file);
  }
}
