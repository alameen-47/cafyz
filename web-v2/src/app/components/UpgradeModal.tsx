...
export function UpgradeModal({ requiredPlan, featurePage, onClose, onGoLicense }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(8px)' }} onClick={onClose}>
      <motion.div
        initial={{ opacity: 0, y: 12, scale: 0.98 }}
        animate={{ opacity: 1, y: 0, scale: 1 }}
        className="w-full max-w-lg rounded-2xl p-6"
        style={{ background: '#0d1326', border: '1px solid rgba(30,127,255,0.15)' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            <p style={{ color: '#6b82a0', fontSize: '0.68rem', letterSpacing: '0.12em', textTransform: 'uppercase' }}>Upgrade required</p>
            <h2 style={{ color: '#e8eef8', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1.25rem', marginTop: 6 }}>
              Unlock {featureLabel}
            </h2>
            <p style={{ color: '#6b82a0', fontSize: '0.82rem', marginTop: 6 }}>
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg" style={{ color: '#6b82a0' }}><X size={16} /></button>
        </div>
            ...
            return (
              <div key={plan} className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
                ...
                <div className="min-w-0 flex-1">
                  <span style={{ color: '#e8eef8', fontWeight: 600, fontSize: '0.85rem' }}>{cfg?.label ?? Meta.label}</span>
                  {priceLine && <p style={{ color: '#6b82a0', fontSize: '0.68rem', marginTop: 2 }}>{priceLine}</p>}
                </div>
                {plan === currentPlan && <span style={{ color: '#6b82a0', fontSize: '0.68rem' }}>Current</span>}
              </div>
            );
          })}
        </div>
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm" style={{ background: 'rgba(255,255,255,0.06)', color: '#a8bdd4' }}>Maybe later</button>
