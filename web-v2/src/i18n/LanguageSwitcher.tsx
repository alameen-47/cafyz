...
export function LanguageSwitcher({ variant = 'header', className = '' }: LanguageSwitcherProps) {
  ...
  return (
    // Endonyms are never translated, and the walker would otherwise cache one
    // language's label as the "source" and force it back after a switch.
    <div ref={ref} data-i18n-ignore className={`relative ${className}`}>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
      ...
      >
        ...
        <span
          lang={current.code}
          dir={current.dir}
          className={`lang-switcher-label truncate ${variant === 'login' ? 'max-w-[4.5rem] sm:max-w-none' : 'max-w-[5.5rem] sm:max-w-none'}`}
        >{btnLabel}</span>
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute end-0 mt-2 z-[80] min-w-[190px] max-h-[62vh] overflow-y-auto overscroll-contain rounded-xl py-1"
            style={{
            ...
            {SUPPORTED_LANGUAGES.map(opt => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2.5 text-start transition-colors hover:bg-[var(--cafyz-accent-soft)]"
                >
                  <span
                    lang={opt.code}
                    dir={opt.dir}
                    style={{ color: 'var(--cafyz-text)', fontSize: '0.88rem', fontWeight: active ? 700 : 500, flex: 1 }}
                  >
                    {opt.nativeLabel}
                  </span>
